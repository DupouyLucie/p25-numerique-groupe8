...
```{code-cell} ipython3
fich=pd.read_csv('data/objects-on-the-moon.csv')
fich.size
```
...
#pd.DataFrame.drop?
#fich.columns
fich=fich.drop(columns=['Unnamed: 0'])
```
...
```{code-cell} ipython3
#pd.DataFrame.dropna?
fich.dropna(how='all',axis='columns', inplace=True )
...
```{code-cell} ipython3
fich.dropna(how='all', inplace=True )
fich
```
...
```{code-cell} ipython3
#pd.to_numeric?
fich[ 'Mass (lb) orig' ]= fich['Mass (lb)']
fich['Mass (lb)']= pd.to_numeric( fich['Mass (lb)'] , errors='coerce')
fich
```
...
```{code-cell} ipython3
fich['Mass (lb)'] =pd.to_numeric(fich['Mass (lb) orig'].str.replace("<", " ").str.replace(">", " "))
...
```{code-cell} ipython3
# votre code
fich['Mass (kg)']= (fich['Mass (lb)'] / 2.205).astype('int')
fich
```
...
fich["Country"].value_counts(normalize=True)
```
...
```{code-cell} ipython3
Objet_US=fich.loc[fich['Country'] == 'United States']
Objet_US['Mass (kg)'].sum()
```
...
```{code-cell} ipython3
fich.loc[fich['Artificial object'].str.contains('Memorial')]
#pd.Series.str?
#Le Luxembourg
```
